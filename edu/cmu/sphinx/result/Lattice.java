...
public class Lattice {
    ...
    public static Lattice readhtk(InputStream stream) throws NumberFormatException, IOException {
        ...
        while ((line = in.readLine()) != null) {
            ...
            if (readingNodes) {
                ...
                if (parts.length != 4 || !parts[0].startsWith("I=") || !parts[1].startsWith("t=") || !parts[2].startsWith("W=")) {
                    in.close();
                    throw new IOException("Unknown node definition: " + line);
                }
                int idx = Integer.parseInt(parts[0].substring(2));
     ...
     *            use only the acoustic scores to compute the posteriors, ignore
     *            the language weight and scores
     * @param acousticModelWeight
     * 			The weight that is given by the noise estimator for multiple acoustic model systems            
	 *
     */
    public void computeNodePosteriors(float languageModelWeightAdjustment, boolean useAcousticScoresOnly, double accousticModelWeight) {
        if (initialNode == null)
            return;
        // forward
        initialNode.setForwardScore(LogMath.LOG_ONE);
        ...
        for (Node currentNode : sortedNodes) {
            for (Edge edge : currentNode.getLeavingEdges()) {
                double forwardProb = edge.getFromNode().getForwardScore()*accousticModelWeight;
                double edgeScore = computeEdgeScore(edge, languageModelWeightAdjustment, useAcousticScoresOnly, accousticModelWeight);
                forwardProb += edgeScore;
                edge.getToNode().setForwardScore(
                        logMath.addAsLinear((float) forwardProb, (float) edge.getToNode().getForwardScore()));
                double vs = edge.getFromNode().getViterbiScore() + edgeScore;
        ...
        while (n.hasPrevious()) {
            ...
            Collection<Edge> currentEdges = currentNode.getLeavingEdges();
            for (Edge edge : currentEdges) {
                double backwardProb = edge.getToNode().getBackwardScore()*accousticModelWeight;
                backwardProb += computeEdgeScore(edge, languageModelWeightAdjustment, useAcousticScoresOnly, accousticModelWeight);
                edge.getFromNode().setBackwardScore(
                        logMath.addAsLinear((float) backwardProb, (float) edge.getFromNode().getBackwardScore()));
            }
        }
     ...
     * @return the score of an edge
     */
    private double computeEdgeScore(Edge edge, float languageModelWeightAdjustment, boolean useAcousticScoresOnly, double acousticModelWeight) {
        if (useAcousticScoresOnly) {
            return edge.getAcousticScore() * acousticModelWeight;
        } else {
            return edge.getAcousticScore() * acousticModelWeight + edge.getLMScore() * languageModelWeightAdjustment;
        }
    }
